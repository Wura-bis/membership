...
export default function MemberForm({ formData, lookups, onChange, onAddressChange, addAddress, removeAddress, isLoading, error, success, onSubmit, submitLabel, onCreateLookup }) {
  return (
    <form onSubmit={onSubmit} aria-label="Member Form">
      ...
      <h2 className="font-semibold mt-4 mb-2">Personal Information</h2>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label htmlFor="phoneNumber">Home Phone</label><input name="phoneNumber" id="phoneNumber" value={formData.phoneNumber || ""} onChange={onChange} className="form-input" /></div>
        <div><label htmlFor="cellPhone">Cell Phone</label><input name="cellPhone" id="cellPhone" value={formData.cellPhone || ""} onChange={onChange} className="form-input" /></div>
      </div>
        ...
        return (
          <div key={idx} className="grid grid-cols-3 gap-2 mb-2">
            <div>
              ...
              <input value={addr.addressLine1} onChange={e => onAddressChange(idx, "addressLine1", e.target.value)} placeholder="Address Line 1" className="form-input" aria-label="Address Line 1" />
            </div>
            <div>
      ...
      {/* Membership Details */}
      <h2 className="font-semibold mt-6 mb-2">Membership Details</h2>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><label htmlFor="categoryId">Member Category</label><CreatableSelect label="" name="categoryId" value={formData.categoryId || ""} options={lookups.categories || []} onChange={onChange} onCreate={val => onCreateLookup('category', val)} placeholder="Type or select..." /></div>
      ...
      {/* Notes, IsActive, Photo Upload, Submit */}
      ...
      <div style={{ marginTop: '32px', display: 'flex', gap: '16px' }}>
        <button type="submit" className="btn-primary" disabled={isLoading} aria-label="Submit Member Form">{submitLabel}</button>
